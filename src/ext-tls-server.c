...
void end_client_connection(int ret) {
    ...
    net_remove_handler(listen_fd.fd, tls_client_handler);
}
...
void tls_server_handler( int rc, int sock ) {
    printf( "TLS: tls_server_handler, rc: %d\n", rc);

    unsigned char client_ip[16] = { 0 };
    size_t cliip_len;
    int ret;

    if( rc <= 0 || listen_fd.fd < 0 ) {
    ...
    if( ret != 0 ) {
        log_warn( "TLS: net_set_(non)block() returned -0x%x", -ret );
        return;
    }
    ...
    net_add_handler( listen_fd.fd, tls_client_handler );
}
...
mbedtls_debug_set_threshold( 0 );
 ...
 mbedtls_entropy_init( &entropy );
    ...
    if( g_sni_entries ) {
        int server_fd = net_bind( "TLS", gconf->dht_addr, "4433" /*gconf->dht_port*/, NULL, IPPROTO_TCP, AF_UNSPEC );
        net_add_handler( server_fd, &tls_server_handler );
        ...
        net_add_handler( listen_fd.fd, &tls_server_handler );
    }
    //mbedtls_ssl_set_bio( &g_ssl, &client_fd, mbedtls_net_send, mbedtls_net_recv, NULL );
    //mbedtls_net_free( &client_fd );
    //mbedtls_ssl_session_reset( &g_ssl );
