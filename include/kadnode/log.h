...
#define log_info(...) if( gconf->verbosity >= LOG_INFO ) { log_print( LOG_INFO, __VA_ARGS__ ); }
#define log_warn(...) if( gconf->verbosity >= LOG_WARNING ) { log_print( LOG_WARNING, __VA_ARGS__ ); }
#ifdef DEBUG
#define log_debug(...) if( gconf->verbosity >= LOG_DEBUG ) { log_print( LOG_DEBUG, __VA_ARGS__ ); }
#else
